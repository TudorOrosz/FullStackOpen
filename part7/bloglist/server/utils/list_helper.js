...
const mostBlogs = (blog_list) => {
  ...
  return topEntry ? { author: topEntry[0], blogs: topEntry[1] } : null;
};
...
const mostLikes = (blog_list) => {
  ...
  const topEntry = _.maxBy(
    Object.entries(likesPerAuthor),
    ([author, likes]) => likes,
  );
  return topEntry ? { author: topEntry[0], likes: topEntry[1] } : null;
};
